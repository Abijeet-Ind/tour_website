...
var hideAlert = function hideAlert() {
  var el = docuement.querySelector('.alert');
...
var displayMap = function displayMap(location) {
  mapboxgl.accessToken = 'pk.eyJ1IjoibW9uamlybyIsImEiOiJjbDF3b2FidncxczFiM2RvZmhiaDMyc3FvIn0.1xj07WwNRMHd6FHTSK534A'; // create map
  ...
  var bounds = new mapboxgl.LngLatBounds(); //LngLatBounds object represents a geographical bounding box, defined by its southwest and northeast points in longitude and latitude.
  ...
  location.forEach(function (loc) {
    ...
    new mapboxgl.Marker({
      element: el,
      anchor: 'bottom'
    }).setLngLat(loc.coordinates).addTo(map); // this adds popup text to the location mark

    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p> Day ".concat(loc.day, ": ").concat(loc.description, "</p>")).addTo(map); // extend mapbound to include current locations
